...
class AddNodeUseCase(private val placementController: IARPlacementController) {
    operator fun invoke(frame: Frame, x: Float, y: Float, imageFileLocation: String): Result<ARNodeData> {
        return placementController.createNodeAt(frame, x, y, imageFileLocation)
    }
}